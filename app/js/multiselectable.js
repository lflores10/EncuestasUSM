	...
	$.fn.multiselectable = function (options) {
		...
		options = $.extend({
			template: '<div class="multiselectable">' +
				...
				'<select multiple="multiple" name="m-selectable"></select>' +
				'</div>' +
				...
				'<select multiple="multiple" name="m-selected"></select>' +
				'</div>' +
			'</div>',
		...
		return that.each(function () {
			...
			var num = 1;
			if ($('.ui-dialog-active .m-selectable').length > 0) {
				while ($('.ui-dialog-active .m-selectable .m-selectable_' + num).length > 0) {
					num++;
				}
