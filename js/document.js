$(document).keydown(function(e){
	///evento sup
	/*if(e.which==46){
		$("div[server-frm].main_widget_hover").each(function(index, element) {
			$(this).remove();
			$.ajax({url:localStorage["server-erp-web-server"]+'include/widgets.php',data:'del='+$(this).text()+'&frm='+$(this).attr('server-frm')});
			return false;
		});
...
function formMarshal(a,d){
	if($("a[server-frm="+(d?d:$(spn[0]+" "+spn[3]).attr("id"))+"]").attr("server-inmate")){
		var e=String(parseInt($("a[server-frm="+(d?d:$(spn[0]+" "+spn[3]).attr("id"))+"]").attr("server-inmate").split("_")[0],16))+String(parseInt($("a[server-frm="+(d?d:$(spn[0]+" "+spn[3]).attr("id"))+"]").attr("server-inmate").split("_")[1],16));
